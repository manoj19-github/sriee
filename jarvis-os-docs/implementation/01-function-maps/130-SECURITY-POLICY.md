...
|---|---|---|---|---|---|---|---|
| 130000 | `shared-security-register-device-mandatory-p0-complete-current-v1` | `registerDeviceIdentity` | Creates per-install device identity and binds it to the interactive user after explicit setup. | OS user/session; local key store | local/db: device public identity | complete/current/v1 | Windows CNG current-user P-256 private material is persisted with export disabled; only the public identity is registry-safe. |
